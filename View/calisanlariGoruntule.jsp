			...
			<div class="row">
				<div class="box-info full animated fadeInDown">
					...
					<div class="additional-btn">
						...
						<ul class="dropdown-menu pull-right animated half fadeInDown"
							...
							<li role="presentation"><a role="menuitem" tabindex="-1"
								href="belediyeCalısanlar.jsp">Tabloya Veri Ekle</a></li>
						</ul>
					...
					<sql:query var="listUrun" dataSource="${database}">
       						 SELECT * FROM belediyecalisanlar
   		 					</sql:query>
							...
							<table data-sortable="" class="table table-striped"
								...
								<thead>
									<tr>
										...
										<th>Nufusa Kayıtlı Olduğu İl</th>
										<th>TC Kimlik Numarası</th>
										<th>Açıklama(Özel Durum vs.)</th>
									</tr>
								</thead>
								<tbody>
									<c:forEach var="calisan" items="${listUrun.rows}">
										<tr>
											<td><c:out value="${calisan.calisanid}" /></td>
											<td><c:out value="${calisan.calisanname}" /></td>
											<td><c:out value="${calisan.calisanbirim}" /></td>
											<td><c:out value="${calisan.calisanil}" /></td>
											<td><c:out value="${calisan.calisantc}" /></td>
											<td><c:out value="${calisan.calisancomment}" /></td>
											<td><a href=calisanGuncelle.jsp?calisanid=${calisan.calisanid}>Güncelle</a></td>
											<td><a href=calisansil?calisanid=${calisan.calisanid}>Sil</a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
